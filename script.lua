...
local function getGardenHorizonsData()
    if not PlayerGui then PlayerGui = LocalPlayer:WaitForChild("PlayerGui", 5) end
...
local function postDataToAPI()
    ...
    updateUI("Status: ✅ API Đã gửi", nil, Color3.fromRGB(150,255,150))
end

local function getNextRestockTimestamp()
    local now = os.time()
    return now + (300 - (now % 300)) + 2
end
...
task.spawn(function()
    print("[GHZ] 🚀 Script v1.8.1 Loaded")
    postDataToAPI()
    while true do
        ...
        while os.time() < targetTs do
            ...
            updateUI(string.format("Status: ⏳ Restock %02d:%02d", math.floor(remaining/60), remaining%60),
                     string.format("Next: %s", fmtUTC(targetTs)), Color3.fromRGB(180, 180, 255))
            task.wait(1)
