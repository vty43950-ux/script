...
-- ── KEYWORDS ────────────────────────────────────────────────────────────────
local STOCK_KEYWORDS = {"stock","seed","gear","shop","restock","item","quantity","supply","available","amount","market","price","cost","store","inventory"}
local ITEM_KEYWORDS  = {"amount","quantity","stock","price","item","name"}
...
local function hasItemData(t)
    ...
    local function scan(tbl, depth)
        ...
        for k, v in pairs(tbl) do
            ...
            for _, ikw in ipairs(ITEM_KEYWORDS) do
                if ks:find(ikw) and not keysFound[ks] then
                    keysFound[ks] = true
                    score = score + 1
                end
            end
    ...
    scan(t, 1)
    return score >= 2 -- At least two item-related keys
end
...
addRow("── Tất cả Remotes (🔵 = Khả nghi) ──", Color3.fromRGB(100, 120, 160))
...
addRow("🎯 Remotes ĐÃ NHẬN STOCK DATA:", Color3.fromRGB(255, 220, 60), true)
...
for _, v in ipairs(remoteList) do
    if v:IsA("RemoteEvent") then
        ...
        v.OnClientEvent:Connect(function(...)
            ...
            local containsItems = false
            for _, arg in ipairs(args) do
                if hasItemData(arg) then containsItems = true; break end
            end

            if containsItems or isRelevant(encoded) or isRelevant(path) then
                if not found[path] then
                    found[path] = true
                    -- Hiện lên scrolling list
                    addRow("🎯 " .. path, Color3.fromRGB(80, 255, 120), true)
                    addRow("   Args: " .. encoded:sub(1, 150), Color3.fromRGB(180, 255, 180))
                    statusLbl.Text = "🎯 Tìm thấy stock! Xem danh sách bên dưới"
