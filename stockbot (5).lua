...
local WEATHER_WHITELIST = {
    ...
    "Acid Rain","Mowis","DJ Kine","Beam Clash","Black Hole","StrangeWeather",
}
...
local function onStockUpdate(seeds, gear, source)
    ...
    if tick() - lastRestockTime < CONFIG.RESTOCK_DEBOUNCE then
        print(ts() .. " [GHZ] Debounce stock"); return
    end

    local ok2, hash = pcall(function()
        return HttpService:JSONEncode({ s = seeds, g = gear })
    end)
    if not ok2 then hash = tostring(#seeds) .. tostring(#gear) end

    if hash == lastStockHash then
        print(ts() .. " [GHZ] Stock duplicate, skip"); return
    end
    ...
    rowSeeds.Text  = "Seeds: " .. #seeds .. " loai"
    rowGear.Text   = "Gear:  " .. #gear  .. " loai"
    rowTime.Text   = os.date("%H:%M:%S")

    postAPI(seeds, gear, currentWeather)
    sendStockWebhook(seeds, gear, currentWeather)
    print(ts() .. string.format(" [GHZ] Stock from [%s]: %d seeds, %d gear", source, #seeds, #gear))
end
    ...
    local function scan(t, depth, ctxCat)
        ...
        for k, v in pairs(t) do
            if type(v) ~= "table" then
                ...
                local isStructureKey = ks:find("shop") or ks:find("data") or ks:find("item") or ks:find("store") or ks:find("market")
                local amt = nil
                if not isStructureKey and type(k) == "string" then
                    amt = v.Amount or v.amount
                       or v.Quantity or v.quantity
                       or v.Stock or v.stock
                    -- Neu khong co Amount thi dung MaxAmount (bieu hien stock toi da = stock hien tai)
                    if amt == nil then
                        amt = v.MaxAmount or v.maxAmount
                    end
                end
